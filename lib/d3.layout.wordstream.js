...
d3.layout.wordStream = function(){
    ...
    var maxFatal = 0,
        minFatal = Number.MAX_SAFE_INTEGER;
    ...
    function buildFontScale(data){
        ...
        let maxFatality = 0;
        let minFatality = Number.MAX_SAFE_INTEGER;

        d3.map(data, function(box){     // thuc hien ham box tren tung phan tu cua data (timestep)
            d3.map(topics, function(topic){     // thuc hien ham topic tren tung phan tu cua topics
                let max = d3.max(box.words[topic], function(d){
                    return d.fatality;         // lay ra freq cao nhat cua 1 record
                });
                let min = d3.min(box.words[topic], function(d){
                    return d.fatality;
                });
                if(maxFatality < max) maxFatality = max;
                if(minFatality > min) minFatality = min;
            })
        });

        maxFatal = maxFatality;
        minFatal = minFatality;
    ...
    wordStream.minFatal = function (_) {
        return arguments.length ? (minFatal = _, wordStream) : minFatal;
    };
    wordStream.maxFatal = function (_) {
        return arguments.length ? (maxFatal = _, wordStream) : maxFatal;
    };
